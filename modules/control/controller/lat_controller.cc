...
Status LatController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  */
  matrix_a_(0, 1) = (1.0);
  matrix_a_(1, 2) = ((2.0*cf_+2.0*cr_)/mass_);
  matrix_a_(2, 3) = (1.0);
  matrix_a_(3, 2) = ((2.0*cf_*lf_-2.0*cr_*lr_)/iz_);

  matrix_a_coeff_ = Matrix::Zero(matrix_size, matrix_size);
  matrix_a_coeff_(1, 1) = -(2.0*cf_*l_f+2.0*cr_*lr_)/mass_;
  matrix_a_coeff_(1, 3) = 2.0*(-cf_*lf_+cr_*lr_)/mass_;
  matrix_a_coeff_(3, 1) = -2.0*(cf_*lf_-cr_*lr_)/iz_;
  matrix_a_coeff_(3, 3) = -2.0*(cf_*lf_+cr_*lr_)/iz_;
...
double LatController::ComputeFeedForward(double ref_curvature) const {
  ...
  if (injector_->vehicle_state()->gear() == canbus::Chassis::GEAR_REVERSE) {
    ...
    steer_angle_feedforwardterm =
        ((lf_+lr_)*ref_curvature + kv*ay -
         matrix_k_(0,2) *
             ((lr_/cf_)/2.0 -
              lf_*mass_*v*v*ref_curvature/(2*cr_(lf_+lr_)))) *
